...
int main(int argc, char** argv)
{
	...
	if(argc != NPOP + 5) // +1 for fitnessFunction.exe, +1 for NPOP. +1 for scaleFactor +1 for fileName
		cout << "Error, number of elements in argument of fitnessFunction.exe is incorrect! Specify NPOP first, then NSEEDS, then the antenna file, then the scale factor, then specify all AraSim output data files, preserving individual order." << endl << endl;
